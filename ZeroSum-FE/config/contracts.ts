...
// Supported Networks
export type NetworkType = 'base' | 'celo' | 'celoMainnet';
...
export const CeloMainnetContractAddresses = {
  ZERO_SUM_SIMPLIFIED: (process.env.NEXT_PUBLIC_CELO_MAINNET_GAME_CONTRACT ||
    '0x3688Ce0123de9583cB39aA90907049Ef4077D810') as `0x${string}`,
  ZERO_SUM_SPECTATOR: (process.env.NEXT_PUBLIC_CELO_MAINNET_SPECTATOR_CONTRACT ||
    '0xe4608436ec4eE31dc63D71af6c06D45E5a15a512') as `0x${string}`,
} as const;
...
export const NETWORK_CONFIG = {
  ...
  celoMainnet: {
    ...
    explorer: process.env.NEXT_PUBLIC_CELO_MAINNET_EXPLORER || 'https://celoscan.io',
    currency: 'CELO',
    contracts: CeloMainnetContractAddresses,
  },
} as const;
...
export const getContractAddresses = (network?: NetworkType) => {
  ...
  const config = NETWORK_CONFIG[activeNetwork] || NETWORK_CONFIG.base;
...
export const getNetworkConfig = (network?: NetworkType) => {
  ...
  return NETWORK_CONFIG[activeNetwork] || NETWORK_CONFIG.base;
};
...
export const CONTRACT_ADDRESSES = {
  ...
  CELO_MAINNET: CeloMainnetContractAddresses,
